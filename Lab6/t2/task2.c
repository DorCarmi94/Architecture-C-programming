...
int searchAndInsertNewVariableNode(char *name, char *value)
{
    ...
    while(answer>0 && currentLink!=NULL)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                currentLink=(mylink*)currentLink->nextNode;
            }
        }
    }
    ...
    if(answer>=0)
    {
        ...
        {
            currentLink->nextNode=(struct link*)newLink;
        }
    }
    else
    {
        free(currentLink->value);
        currentLink->value=calloc(strlen(value),sizeof(char));
        int i;
...
void freeVarsList()
{
    ...
    while (currentLink!=NULL)
    {
        nextLink=(mylink*)currentLink->nextNode;
...
int main(int argc, char **argv) {
    ...
    {  
        ...
        else if(strcmp(lineFromTheUser,"procs\n")==0)
        {
            ...
            //Substitution
            int kk=0;
            for(kk=0; kk<nextCommand->argCount; kk++)
            {
                if(nextCommand->arguments[kk][0]=='$')
                {
                    char *name=&(nextCommand->arguments[kk][1]);
                    char *value=getValueByName(name);
                    if(value==NULL)
                    {
                        printf("problem replacing variable %s\n",name);
                    }
                    replaceCmdArg(nextCommand,kk,value);                    
                }
            }

            if(strcmp(nextCommand->arguments[0],"set")==0)
            {
                printf("starting assignemnt\n");
                int ans=searchAndInsertNewVariableNode(nextCommand->arguments[1],nextCommand-> arguments[2]);
                if(ans>0)
                {
                    printf("Weeehoooo it worked, and the new node is there\n");

                }
                else
                {
                    printf("May be yes? May be not? confused? so do I\n");

                }
                freeCmdLines(nextCommand);
                
            }
            else if(strcmp(nextCommand->arguments[0],"vars")==0)
            {
                printVarsList();
                freeCmdLines(nextCommand);
            }
            
            else if(strcmp(nextCommand->arguments[0],"cd")==0)
            {
                if(strncmp(nextCommand->arguments[1],"~",1)==0)
                {
                    ...
                    if(nextCommand->arguments[1][1]=='/')
                    {
                        char *restOfTheAddress=&nextCommand->arguments[1][2];
                        int ans2=chdir(restOfTheAddress);
                        if(ans2==-1)
                        {
                            perror("chdir: ");
                        }
...
void execute(process** process_list,cmdLine *pCmdLine, int debugMode)
{
    ...
    if (pid == -1) {
        ...
        if(pCmdLine->inputRedirect!=NULL)
        {
            const char *fileName=pCmdLine->inputRedirect;
            fclose(fileName);
            fopen(stdin,"r");
        }
        if(pCmdLine->outputRedirect!=NULL)
